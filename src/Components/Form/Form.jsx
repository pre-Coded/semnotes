...
const Form = () => {
    ...
    const [details, setDetails] = useState({ year : "", branch : "", sem : "", sub : "", data : "", type : ""});
  ...
  return (
    <div className='fade-bg relative text-white hide-scrollbar overflow-y-scroll p-2 h-full w-full scroll-smooth'>
        ...
        <div className='relative w-full h-full p-2 flex flex-col space-y-10'>
                ...
                <div className="relative w-full flex justify-center items-center">
                    <h1 className="w-full bg-white brightness-150 text-black text-xl absolute flex items-center justify-center rounded-lg shadow-lg py-3">{details.type ? "Type " + details.type : <span className='flex items-center space-x-4'><span>Select Type</span> <MdArrowDropDownCircle/></span>}</h1>
                    <select onChange={handleDetails} name="type" className='outline-none text-white rounded-md w-full opacity-0' id="">
                        <option value="N/A">Select Type</option>
                        {
                            type.map((type) =>{
                                return <option value={type}>Type {type}</option>
                            })
                        }
                    </select>
                </div> 
